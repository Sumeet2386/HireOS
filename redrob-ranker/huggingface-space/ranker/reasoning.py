...
def _get_high_signal_skills(candidate: dict[str, Any], n: int = 3) -> list[str]:
    """Get skills specifically matching JD's core requirements (embeddings, vector DBs, ranking)."""
    skills = candidate.get("skills", [])
...
def _get_career_trajectory(candidate: dict[str, Any]) -> str | None:
    ...
    if current_title and prev_title and current_company != prev_company:
        return f"progressed from {prev_title} at {prev_company} to {current_title} at {current_company}"
    return None
...
def _extract_quantitative_achievements(candidate: dict[str, Any]) -> str | None:
    ...
    for job in career[:3]:  # Check most recent 3 roles
        desc = job.get("description", "")
        # Look for percentage improvements, scale numbers, etc.
        patterns = [
            r'reduced .+? by \d+%',
            r'improved .+? by \d+%',
            r'increased .+? by \d+%',
...
def build_template_reasoning(
    ...
    structure_variant = int(hashlib.md5(cid.encode()).hexdigest()[:4], 16) % 5
    ...
    if structure_variant == 0:
        # Pattern: Title-first with skills
        ...
        # Pattern: Skills-first with trajectory
        ...
        # Pattern: Company/trajectory-first
        if trajectory:
            sentence1 = (
        ...
        # Pattern: Engagement-led (for highly responsive candidates)
    ...
    consulting_ratio = sum(
        1 for c in companies if _fuzzy_match_set(c, CONSULTING_FIRMS)
    ) / max(1, len(companies))
    ...
    # Title mismatch for lower tiers
